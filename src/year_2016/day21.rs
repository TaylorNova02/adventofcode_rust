...
pub fn solve(input: &str, s1: &str, s2: &str) {
    ...
    assert_eq!(
        part1("rotate based on position of letter b", "abdec"),
        "ecabd"
    );
    assert_eq!(
        part1("rotate based on position of letter d", "ecabd"),
        "decab"
    );
...
fn part1(input: &str, s: &str) -> String {
    ...
    for line in input.trim().split('\n') {
        ...
        if let Some(cap) = SWAP_POS.captures(line) {
            ...
            ops.push(Op::SwapLetter(
                cap.get(1).unwrap().as_str().as_bytes()[0],
